...
function handle_statements(statements, opts, nodes) {
    statements.forEach(function(statement) {
        if (statement.type === 'MustacheStatement' || statement.type === 'SubExpression') {
            ...
            handle_partial(statement, opts, nodes);
...
function handle_partial(partial, opts, nodes) {
    var pairs = partial.hash && partial.hash.pairs || [];
    var hashStatements = pairs.map(function(pair) {
        return pair.value;
    });
    handle_statements(hashStatements, opts, nodes);
}
